...
public class BarbellRowController {
    ...
    @ApiParam()
    @ApiResponses({
            @ApiResponse(code = 200, message = "한 세트에 대한 barbell row 값이 정상적으로 입력됨"),
    ...
    @ApiOperation(value = "barbell row과 관련된 전체 데이터를 조회하는 기능"
            , notes = "사용자로부터 첫 기록부터 마지막 기록까지 기입된 barbell-row에 대한 모든 정보를 반환하도록 한다. id, weight, count가 담긴 리스트를 반환한다. id는 세트 id를 의미한다.")
    @ApiParam()
    @ApiResponses({
            @ApiResponse(code = 200, message = "전체 barbell row에 대한 값이 조회됨"),
            @ApiResponse(code = 500, message = "server error")
    })
    @GetMapping()
    ...
    @ApiParam()
    @ApiResponses({
            @ApiResponse(code = 200, message = "특정 barbell row에 대한 값이 조회됨"),
            @ApiResponse(code = 500, message = "server error")
    })
    @GetMapping("/{id}")
    ...
    @ApiOperation(value = "barbell row했을 때 최고 횟수를 조회하는 기능"
            , notes = "사용자로부터 첫 기록부터 마지막 기록 사이에 기입된 barbell-row 세트 중에서 가장 많이 수행한 횟수가 있는 정보를 반환하도록 한다. 반환되는 정보에는 id, weight, count, start_time, end_time, date 가 포함된다. (나중에는 해당 세트를 수행한 날짜에서 기록한 모든 barbell row정보를 가져오는 것도 고려)")
    @ApiParam()
    @ApiResponses({
            @ApiResponse(code = 200, message = "barbell row 최고 횟수에 대한 값이 조회됨"),
            @ApiResponse(code = 500, message = "server error")
    })
    ...
    @ApiOperation(value = "barbell row했을 때 최저 횟수를 조회하는 기능"
            , notes = "사용자로부터 첫 기록부터 마지막 기록 사이에 기입된 barbell-row 세트 중에서 가장 적게 수행한 횟수가 있는 정보를 반환하도록 한다. 반환되는 정보에는 id, weight, count, start_time, end_time, date 가 포함된다. (나중에는 해당 세트를 수행한 날짜에서 기록한 모든 barbell row정보를 가져오는 것도 고려)")
    @ApiParam()
    @ApiResponses({
            @ApiResponse(code = 200, message = "barbell row 최고 횟수에 대한 값이 조회됨"),
            @ApiResponse(code = 500, message = "server error")
    ...
    @ApiOperation(value = "barbell row했을 때 최고 중량을 조회하는 기능"
            , notes = "사용자로부터 첫 기록부터 마지막 기록 사이에 기입된 barbell-row 세트 중에서 가장 무거운 중량을 친 정보를 반환하도록 한다. 반환되는 정보에는 id, weight, count, start_time, end_time, date 가 포함된다. (나중에는 해당 세트를 수행한 날짜에서 기록한 모든 barbell row정보를 가져오는 것도 고려)")
    @ApiParam()
    @ApiResponses({
            @ApiResponse(code = 200, message = "barbell row 최고 중량에 대한 값이 조회됨"),
            @ApiResponse(code = 500, message = "server error")
    })
    ...
    @ApiOperation(value = "barbell row했을 때 최저 중량을 조회하는 기능"
            , notes = "사용자로부터 첫 기록부터 마지막 기록 사이에 기입된 barbell-row 세트 중에서 가장 가벼운 중량을 친 정보를 반환하도록 한다. 반환되는 정보에는 id, weight, count, start_time, end_time, date 가 포함된다. (나중에는 해당 세트를 수행한 날짜에서 기록한 모든 barbell row정보를 가져오는 것도 고려)")
    @ApiParam()
    @ApiResponses({
            @ApiResponse(code = 200, message = "barbell row 최저 중량에 대한 값이 조회됨"),
            @ApiResponse(code = 500, message = "server error")
    })
    ...
    @ApiParam()
    @ApiResponses({
            @ApiResponse(code = 200, message = "barbell row 데이터가 정상적으로 업데이트 됨"),
            @ApiResponse(code = 500, message = "server error")
    })
    @PutMapping("/{id}")
    ...
    @ApiParam()
    @ApiResponses({
            @ApiResponse(code = 200, message = "barbell row 데이터가 정상적으로 제거 됨"),
            @ApiResponse(code = 500, message = "server error")
    })
    @DeleteMapping("/{id}")
